					...
					<form action="/dept/update" method="post">
						...
						<div>
							...
							<input type="text" value="${dto.departmentName}"
								placeholder="부서명 입력" name="departmentName">
						</div>
						<div>
							<label>관리자 아이디</label> <input type="text"
								value="${dto.managerId}" name="managerId">
						</div>
						<div>
							<label>지역 코드</label> <input type="text" value="${dto.locationId}"
								name="locationId">
						</div>
						<button type="submit" class="btn btn-outline-primary">부서 수정</button>
